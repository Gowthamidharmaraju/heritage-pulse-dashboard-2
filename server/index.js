...
// Diagnostic Google Drive Test Endpoint
app.get('/api/test-drive', async (req, res) => {
  try {
    const driveService = require('./googleDriveService');
    const rootId = driveService.rootFolderId || process.env.GOOGLE_DRIVE_FOLDER_ID || '1fyOaEebMIdQN1Ke2oLXhtl_tca8auuvb';
    const hasClientId = !!process.env.GOOGLE_DRIVE_CLIENT_ID;
    const hasSecret = !!process.env.GOOGLE_DRIVE_CLIENT_SECRET;
    const hasToken = !!process.env.GOOGLE_DRIVE_REFRESH_TOKEN;
    const hasServiceAccount = !!process.env.GOOGLE_DRIVE_CLIENT_EMAIL;

    const testItem = {
      id: `HP-TEST-${Date.now()}`,
      title: 'Google Drive Sync Diagnostic Test',
      category: 'System Test',
      status: 'PUBLISHED',
      publishing_date: new Date().toISOString().split('T')[0],
      body: 'This is a test article generated by Heritage Pulse Dashboard system to verify Google Drive sync.'
    };

    const syncResult = await driveService.syncArticleToDrive(testItem);

    res.json({
      success: true,
      message: 'Google Drive sync test completed successfully!',
      rootFolderId: rootId,
      credentials: {
        oauth2: { hasClientId, hasSecret, hasToken },
    ...
    res.status(500).json({
      ...
      error: err.message,
      stack: err.stack,
      rootFolderId: process.env.GOOGLE_DRIVE_FOLDER_ID || '1fyOaEebMIdQN1Ke2oLXhtl_tca8auuvb',
      credentialsPresent: {
